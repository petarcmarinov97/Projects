                        ...
                        <div className="header_poster_wrapper">
                            <section className="header_poster">
                                <div className="title" dir="auto">
                                    <h2 className="16">
                                    {movie.title
                                    ? <a href={`/tv/details/${movie.id}`}>{movie.title}</a>
                                    :<a href={`/tv/details/${movie.id}`}>{movie.name}</a>
                                    }

                                    {movie.release_date
                                    ? <span className="release_date">({movie.release_date})</span>
                                    : <span className="release_date">({movie.first_air_date})</span>
                                    }
                                    ...
                                    : <span className="release">({movie.first_air_date})</span>
                                        ...
                                        <span className="genres">
                                            {genres.map(genre=>(
                                                <a key={genre.id} href={`/tv/details/${movie.id}`}>{genre.name} </a>
                                            ))}
                                        </span>
                                        <span className="runtime">{movie.episode_run_time} m</span>
                                    </div>
                                </div>
                                <ul className="auto_actions">
                                    <li className="tooltip_use" data-role="tooltip">
                                        <a id="favaourite" href="#" className="add_to_account_favourite">
                                            <span className="heartIcon"></span>
                                        </a>
                                    </li>
                                    <li className="tooltip_use_second" data-role="tooltip">
