...
import '../../../Styles/MoviePage/DetailsCard.css';
                        ...
                        <div className="header_poster_wrapper">
                            <section className="header_poster">
                                <div className="title" dir="auto">
                                    <h2 className="16">
                                    {movie.title
                                    ? <a href={`/movie/details/${movie.id}`} title={movie.title}>{movie.title}</a>
                                    :<a href={`/movie/details/${movie.id}`} title={movie.name}>{movie.name}</a>
                                    }
                                    ...
                                    <div className="facts">
                                        ...
                                        <span className="genres">
                                            {genres.map(genre=>(
                                                <a key={genre.id} href={`/movie/details/${movie.id}`}>{genre.name} </a>
                                            ))}
                                        </span>
